...
def overview_mass_added_plotter(source, tooltips):
    p = figure(
        title="Mean Added Mass over HDX Timeseries, Colored by RTxDT Error in ms ",
        plot_height=350,
        plot_width=1300,
        background_fill_color="whitesmoke",
        tools="pan,wheel_zoom,box_zoom,hover,save,reset,help",
        tooltips=tooltips,
    )
    err_low, err_high = min(source.data["rtxdt_rmse"]), max(source.data["rtxdt_rmse"])
    mapper = linear_cmap(
        field_name="rtxdt_rmse", palette=Spectral6, low=err_low, high=2
    )
...
def overview_error_plotter(source, tooltips):
    p = figure(
        title="RMSEs of RT-Group RT and DT Center-of-Mass Errors",
        plot_height=350,
        plot_width=1300,
        background_fill_color="whitesmoke",
        tools="pan,wheel_zoom,box_zoom,hover,save,reset,help",
...
winner_paths = sorted(
    [path for path in snakemake.input if "winner.cpickle.zlib" in path]
)
undeut_ground_paths = sorted(
    [path for path in snakemake.input if "undeut_grounds" in path]
)
winner_score_paths = sorted(
    [path for path in snakemake.input if "winner_scores" in path]
)
rtdt_cv_paths = sorted([path for path in snakemake.input if "rtdt_com_cvs" in path])

pnames = sorted(
    set(["_".join(fn.split("/")[-1].split("_")[:3]) for fn in winner_paths])
)
snames = sorted(
    set(["_".join(fn.split("/")[-1].split("_")[:4]) for fn in winner_paths])
)
...
for winner_path in winner_paths:
    ...
    for ic in winners:
        ...
        rtxdt_errs.append(
            ((ic.rt_ground_err * 0.07) * ic.dt_ground_err) ** 2
        )  # 0.07 approximately scales rt-bins to ms
...
idotps = (
    []
)  # idotp = isotopic dot product: the dot product of the observed and theoretical added mass distributions of an isotopic cluster
...
if snakemake.config["old_data_dir"] is not None:
    ...
    for file in glob.iglob(
        "resources/old_data/*.pickle"
    ):  # TODO: This should be controlled by a run-variable in config
    ...
    g.add_legend()
    plt.savefig(
        "results/plots/" + snakemake.config["run_name"] + "_overview_pairplot.png"
    )  # TODO: This should be a snakemake.output value
else:
...
final = column(
    Div(
        text="<h1 style='margin-left: 250px;'>Overview of "
        + snakemake.config["run_name"]
        + "</h1>"
    ),
    overview_mass_added_plotter(source, tooltips),
    overview_error_plotter(source, tooltips),
    Div(
        text="<img source='plots/"
        + snakemake.config["run_name"]
        + "_overview_pairplot.png' width='1300' height='1300>'"
    ),
)
