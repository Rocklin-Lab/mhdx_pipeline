...
auxiliary_spec = importlib.util.spec_from_file_location("auxiliary.py", "./auxiliary/rate_fitting/auxiliary.py")
hx_rate_fit_functions_spec = importlib.util.spec_from_file_location("molmass.py", "./auxiliary/rate_fitting/hx_rate_fit_functions.py")
...
from auxiliary import calc_rmse, load_pickle_file, calculate_theoretical_isotope_dist_from_sequence
...
def fit_hx_rates_optimize(timepoints, thr_isotope_dist_list, exp_isotope_dist_list, num_rates, backexchange, num_bins=None, n_iter=200,
                 temp=0.00003, step_size=0.02):
    """
...
def write_hx_rate_output(intrinsic_rates, fitted_rates, output_path, pdb_name, pD, temp, backexchange, temp_bh,
                         step_size_bh, n_iter_bh):
    """
    ...
    param_data = '{},{},{},{},{},{},{}\n'.format(pdb_name, str(pD), str(temp), str(backexchange), str(temp_bh),
                                                 str(n_iter_bh), str(step_size_bh))
...
def fit_hx_rates(pdb_path, timepoint_list, ms_data_list, output_path, num_bins=50, pD= 6.15, temp=298, nterm=None,
                 cterm=None, temp_bh=0.0003, step_size_bh=0.02, n_iter_bh=200, write_output=False, plot_rate=False):
    """
    ...
    theoretical_isotope_dist = calculate_theoretical_isotope_dist_from_sequence(sequence)
    ...
    fit_rates = fit_hx_rates_optimize(timepoints=timepoint_list,
    ...
    if write_output:
        ...
        write_hx_rate_output(sorted_intrinsic_rates, sorted_fitted_hx_rates, output_path, pdb_name, pD, temp, back_exchange,
                             temp_bh, step_size_bh, n_iter_bh)
    ...
    if plot_rate:
        ...
        pdb_name = pdb_path_components[1]
        sorted_intrinsic_rates = sorted(np.log(intrinsic_rates[intrinsic_rates > 0]))
        plot_rates(sorted_intrinsic_rates, sorted_fitted_hx_rates, output_path, pdb_name)
