...
def snakemake_makeplot(prefix, inputs, output):
    ...
    for i, x in enumerate(winner):
        ...
        for runner in runners[i]:
            if (runner.dt_ground_fit > x.dt_ground_fit and
                runner.rt_ground_fit > x.rt_ground_fit and
                abs(runner.log_baseline_auc - undeut_grounds[0][runner.charge_states[0]].log_baseline_auc) <
                abs(x.log_baseline_auc - undeut_grounds[0][x.charge_states[0]].log_baseline_auc))   :
                
                plt.plot(runner.baseline_integrated_mz / max(runner.baseline_integrated_mz))
        ...
        if i == 0:
            ...
            plt.text(1.0,0.1,'best idotp +%.0f %.3f' % (idotp_q[max_idotp], max_idotp),horizontalalignment='right',verticalalignment='bottom',transform=ax.transAxes)
            
        
        plt.text(1.0,1.0,'+%.0f, %s factors' % (x.charge_states[0], x.n_factors),horizontalalignment='right',verticalalignment='top',transform=ax.transAxes)
        
        plt.text(0.01,1.0,str(i) , horizontalalignment='left',verticalalignment='top',transform=ax.transAxes)
        ...
        plt.yticks([])
        plt.xticks([])
        if i == 0: plt.title("Raw isotopic distribution")
        sns.despine()
        ...
        plt.yticks([])
        plt.xticks([])
        if i == 0: plt.title("Overlapped peaks")
        sns.despine()
        
        ax=plt.subplot(len(winner), 9, (9*i)+7)
        plt.plot(x.rts / max(x.rts))
        plt.plot(undeut_grounds[0][x.charge_states[0]].rts / max(undeut_grounds[0][x.charge_states[0]].rts),
                 color='red')
        plt.yticks([])
        plt.xticks([])
        plt.text(1.0,1.0,'%.3f' % x.rt_ground_fit,
                 horizontalalignment='right',verticalalignment='top',transform=ax.transAxes)
        plt.ylim(0,1.4)
        if i == 0: plt.title("RT (red=undeut)")
        #sns.despine()
        
        ax=plt.subplot(len(winner), 9, (9*i)+8)
        plt.plot(x.dts / max(x.dts))
        plt.plot(undeut_grounds[0][x.charge_states[0]].dts / max(undeut_grounds[0][x.charge_states[0]].dts),
                 color='red')
        plt.text(1.0,1.0,'%.3f' % x.dt_ground_fit,
                 horizontalalignment='right',verticalalignment='top',transform=ax.transAxes)
        plt.yticks([])
        plt.xticks([])
        plt.ylim(0,1.4)
        if i == 0: plt.title("Drift time")
        #sns.despine()
        
        ax=plt.subplot(len(winner), 9, (9*i)+9)
        plt.plot(x.dts / max(x.dts))
        plt.bar([0, 1],
                [sum(undeut_grounds[0][x.charge_states[0]].baseline_subtracted_mz) * undeut_grounds[0][x.charge_states[0]].outer_rtdt,
                 sum(x.baseline_subtracted_mz) * x.outer_rtdt],
                 color=['red', 'blue'])
        plt.yticks([])
        plt.xticks([])
        plt.xlim(-0.5,5)
        plt.text(1.0,1.0,'%.1e' % (sum(undeut_grounds[0][x.charge_states[0]].baseline_subtracted_mz) * undeut_grounds[0][x.charge_states[0]].outer_rtdt),
                 horizontalalignment='right',verticalalignment='top',transform=ax.transAxes,color='red')
        plt.text(1.0,1.0,'\n%.1e' % (sum(x.baseline_subtracted_mz)  * x.outer_rtdt),
                 horizontalalignment='right',verticalalignment='top',transform=ax.transAxes,color='blue')
        sns.despine()
        if i == 0: plt.title("Magnitude")
