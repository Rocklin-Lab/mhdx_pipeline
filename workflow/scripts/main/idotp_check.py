...
est_peak_gaps = (
    [0]
    + list(np.linspace(c13_mass_diff, hd_mass_diff, 7))
    + [hd_mass_diff for x in range(total_isotopes - 8)]
)
cum_peak_gaps = np.cumsum(est_peak_gaps)

mz_lows = library_info["obs_mz"].values[i] - (
    low_mass_margin / library_info["charge"].values[i]
)
mz_highs = library_info["obs_mz"].values[i] - (
    total_isotopes / library_info["charge"].values[i]
)
mz_centers = library_info["obs_mz"].values[i] + (
    cum_peak_gaps / library_info["charge"].values[i]
)
...
for fn in ins:
    ...
    [
        gauss_undeut_ics.append(ic)
        for i in range(len(newDataTensor.factors))
        for ic in newDataTensor.factors[i].isotope_clusters
    ]
...
for ic in gauss_undeut_ics:
    df = pd.DataFrame(
        ic.baseline_integrated_mz, columns=["major_species_integrated_intensities"]
    )
...
best_factor_idotp = [
    max(
        [
            hx_prot.calculate_isotope_dist(
                pd.DataFrame(frame, columns=["major_species_integrated_intensities"])
            )
            for factor in factor_frames
            for frame in factor
        ]
    ),
    np.argmax(
    ...
    pd.DataFrame(
        {
            ...
            "charge": [charge],
            "idotp": [best_idotp],
            "factor_idotp": [best_factor_idotp],
            "best": [dot_products[best_idotp_idx][2]],
            "ic_mzs": [charge_ic_int_mzs],
            "factor_mzs": [factors],
            "n_factors": [n_factors],
        }
    ),
