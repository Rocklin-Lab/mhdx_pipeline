...
def hx_of_t(t, k, backexchange=0.9, D2O_purity=0.95, D2O_fraction=0.9):
    return (1.0 - np.exp(-k * t)) * (
        D2O_fraction * D2O_purity * backexchange
    )  # D20 fraction * D20 purity * backexchange


def hx_of_t_alt(t, k, backexchange=0.001, D2O_purity=0.95, D2O_fraction=0.9):
    return (
        (1.0 - np.exp(-k * t))
        * (np.exp(-k * backexchange))
...
def rates_from_fes(p, fes, T):
    ...
    fractions = np.exp(-fes_temp / (Rconstant * T)) / (
        1.0 + np.exp(-fes_temp / (Rconstant * T))
    )
...
def isotope_poibin_dist_rates_at_timepoints(
    timepoints, rates, nbins, isotope_dist, backexchange
):
    out = np.zeros((len(timepoints), nbins))
    for i, timepoint in enumerate(timepoints):
        out[i, :] = isotope_poibin_dist_rates(
            isotope_dist, timepoint, rates, nbins=nbins, backexchange=backexchange[i]
        )
    return np.ravel(out)
...
def get_concat_data(timepoints, data):
    ...
    for timepoint, dataset in zip(timepoints, data):
        concat_data += list(
            dataset["major_species_integrated_intensities"]
            / max(dataset["major_species_integrated_intensities"])
        )
...
def abego_string(structure):
    ...
    for i in range(len(residues)):
        if i + 1 == len(residues):
            ...
            if i > 0:
                aminus1 = residues[i - 1]["C"].get_vector()
    ...
    for i in range(1, len(abego_string) - 1):
        if abego_string[i] in "GO":
            ...
            for partner in [-5, -4, -3, 3, 4, 5]:
                if i + partner < 0 or i + partner >= len(abego_string):
                    continue
                if partner < 0:
            ...
            if hbond_partners[i] != None and i in hbond_partners:
                if (
                    abego_string[hbond_partners[i]] in "BE"
                    and abego_string[hbond_partners.index(i)] in "BE"
                ):
                    nextaa = "E"
...
def plot_decay(p, xs, fes=None, rates=None, color=None, backexchange=0.9, label=""):
    if fes == None:
        fes = np.array([-5 for x in p.sequence])
    if rates == None:
        rates = p.rates
    plt.semilogx(
        xs,
        rate_with_fe(xs, rates, fes, backexchange=backexchange),
        color=color,
        label=label,
    )
...
class hxprot:
    def calc_pred_kc(self, pD, temp):
        ...
        bai_data["ka"], bai_data["kb"], bai_data["kw"] = 6.95e-01, 1.87e08, 5.27e-04
        bai_data["R"] = 1.987
        bai_data["pKc_D"] = (
            np.log10(
                (10 ** -4.48)
                * np.exp(-1000 * (((1.0 / temp) - (1.0 / 278)) / bai_data["R"]))
            )
            * -1
        )
        bai_data["pKc_E"] = (
            np.log10(
                (10 ** -4.93)
                * np.exp(-1083 * (((1.0 / temp) - (1.0 / 278)) / bai_data["R"]))
            )
            * -1
        )
        bai_data["pKc_H"] = (
            np.log10(
                (10 ** -7.42)
                * np.exp(-7500 * (((1.0 / temp) - (1.0 / 278)) / bai_data["R"]))
            )
            * -1
        )
        ...
        bai_data["acid_left"] = {
            x.split()[0]: float(x.split()[1]) for x in bai_raw_data
        }
        bai_data["acid_right"] = {
            x.split()[0]: float(x.split()[2]) for x in bai_raw_data
        }
        bai_data["base_left"] = {
            x.split()[0]: float(x.split()[3]) for x in bai_raw_data
        }
        bai_data["base_right"] = {
            x.split()[0]: float(x.split()[4]) for x in bai_raw_data
        }

        bai_data["acid_left"]["D"] = np.log10(
            10 ** (-0.9 - pD) / (10 ** (-bai_data["pKc_D"]) + 10 ** (-pD))
            + 10 ** (0.9 - bai_data["pKc_D"])
            / (10 ** (-bai_data["pKc_D"]) + 10 ** (-pD))
        )
        bai_data["acid_left"]["E"] = np.log10(
            10 ** (-0.6 - pD) / (10 ** (-bai_data["pKc_E"]) + 10 ** (-pD))
            + 10 ** (-0.9 - bai_data["pKc_E"])
            / (10 ** (-bai_data["pKc_E"]) + 10 ** (-pD))
        )
        bai_data["acid_left"]["H"] = np.log10(
            10 ** (-0.8 - pD) / (10 ** (-bai_data["pKc_H"]) + 10 ** (-pD))
            + 10 ** (0 - bai_data["pKc_H"]) / (10 ** (-bai_data["pKc_H"]) + 10 ** (-pD))
        )
        bai_data["acid_left"]["CT"] = np.log10(
            10 ** (-0.05 - pD) / (10 ** (-bai_data["pKc_E"]) + 10 ** (-pD))
            + 10 ** (0.96 - bai_data["pKc_E"])
            / (10 ** (-bai_data["pKc_E"]) + 10 ** (-pD))
        )

        bai_data["acid_right"]["D"] = np.log10(
            10 ** (-0.12 - pD) / (10 ** (-bai_data["pKc_D"]) + 10 ** (-pD))
            + 10 ** (0.58 - bai_data["pKc_D"])
            / (10 ** (-bai_data["pKc_D"]) + 10 ** (-pD))
        )
        bai_data["acid_right"]["E"] = np.log10(
            10 ** (-0.27 - pD) / (10 ** (-bai_data["pKc_E"]) + 10 ** (-pD))
            + 10 ** (0.31 - bai_data["pKc_E"])
            / (10 ** (-bai_data["pKc_E"]) + 10 ** (-pD))
        )
        bai_data["acid_right"]["H"] = np.log10(
            10 ** (-0.51 - pD) / (10 ** (-bai_data["pKc_H"]) + 10 ** (-pD))
            + 10 ** (0 - bai_data["pKc_H"]) / (10 ** (-bai_data["pKc_H"]) + 10 ** (-pD))
        )

        bai_data["base_left"]["D"] = np.log10(
            10 ** (0.69 - pD) / (10 ** (-bai_data["pKc_D"]) + 10 ** (-pD))
            + 10 ** (0.10 - bai_data["pKc_D"])
            / (10 ** (-bai_data["pKc_D"]) + 10 ** (-pD))
        )
        bai_data["base_left"]["E"] = np.log10(
            10 ** (0.24 - pD) / (10 ** (-bai_data["pKc_E"]) + 10 ** (-pD))
            + 10 ** (-0.11 - bai_data["pKc_E"])
            / (10 ** (-bai_data["pKc_E"]) + 10 ** (-pD))
        )
        bai_data["base_left"]["H"] = np.log10(
            10 ** (0.80 - pD) / (10 ** (-bai_data["pKc_H"]) + 10 ** (-pD))
            + 10 ** (-0.10 - bai_data["pKc_H"])
            / (10 ** (-bai_data["pKc_H"]) + 10 ** (-pD))
        )

        bai_data["base_right"]["D"] = np.log10(
            10 ** (0.60 - pD) / (10 ** (-bai_data["pKc_D"]) + 10 ** (-pD))
            + 10 ** (-0.18 - bai_data["pKc_D"])
            / (10 ** (-bai_data["pKc_D"]) + 10 ** (-pD))
        )
        bai_data["base_right"]["E"] = np.log10(
            10 ** (0.39 - pD) / (10 ** (-bai_data["pKc_E"]) + 10 ** (-pD))
            + 10 ** (-0.15 - bai_data["pKc_E"])
            / (10 ** (-bai_data["pKc_E"]) + 10 ** (-pD))
        )
        bai_data["base_right"]["H"] = np.log10(
            10 ** (0.83 - pD) / (10 ** (-bai_data["pKc_H"]) + 10 ** (-pD))
            + 10 ** (0.14 - bai_data["pKc_H"])
            / (10 ** (-bai_data["pKc_H"]) + 10 ** (-pD))
        )
        ...
        bai_data["Fta"] = np.exp(
            -1 * 14000 * ((1.0 / temp) - (1.0 / 293)) / bai_data["R"]
        )
        bai_data["Ftb"] = np.exp(
            -1 * 17000 * ((1.0 / temp) - (1.0 / 293)) / bai_data["R"]
        )
        bai_data["Ftw"] = np.exp(
            -1 * 19000 * ((1.0 / temp) - (1.0 / 293)) / bai_data["R"]
        )
        ...
        for i, aa in enumerate(self.sequence):
            if i == 0 or aa in ["P"]:
                ...
                Fa = 10.0 ** (
                    bai_data["acid_left"][aa] + bai_data["acid_right"][seq[i - 1]]
                )
                Fb = 10.0 ** (
                    bai_data["base_left"][aa] + bai_data["base_right"][seq[i - 1]]
                )
                if i == 1:
                ...
                Fa_ka_D_Fta = (
                    Fa * bai_data["ka"] * bai_data["conc_Dplus"] * bai_data["Fta"]
                )
                Fb_kb_OD_Ftb = (
                    Fb * bai_data["kb"] * bai_data["conc_ODminus"] * bai_data["Ftb"]
                )
    ...
    def calc_fe_from_rate_grid(self, meas_rates):
        ...
        for pos_index, int_rate in enumerate(int_rates):
            for rate_index, meas_rate in enumerate(meas_rates):
                out[pos_index][rate_index] = fe_from_rate(
                    int_rate, meas_rate, self.temp
                )
        return out
    ...
    def __init__(
        ...
        if pdbfilename != "":
            ...
            residues = [res for res in model.get_residues()]
            seq = "".join(
                [Polypeptide.three_to_one(aa.get_resname()) for aa in residues]
            )

        self.sequence = Nterm + seq + Cterm
        self.allowed = np.array(
            [False for x in Nterm]
            + [False if x == "P" else True for x in seq]
            + [False for x in Cterm]
        )
        ...
        self.h_indices = np.array(
            [x for x in range(len(self.rates)) if self.rates[x] > 0]
        )
        self.blank_fes = np.zeros(len(self.sequence))
        self.distance_to_nonpolar = distance_to_nonpolar(self.sequence)
        self.active_index = np.array(
            [sum(self.allowed[0:x]) for x in range(len(self.allowed))], dtype=object
        )
        for i in range(len(self.active_index) - 1):
            if self.active_index[i + 1] == self.active_index[i]:
                self.active_index[i] = None
        self.idx_to_full = np.array(
            [list(self.active_index).index(i) for i in range(sum(self.allowed))]
        )
        ...
        if pdbfilename != "":
            ...
            for i, res in enumerate(residues):
                ...
                atoms_in_radius.append(
                    sum(
                        [
                            smooth_dist_cutoff(
                                np.linalg.norm(
                                    res["N"].coord.astype(float) - j.coord.astype(float)
                                )
                            )
                            for j in model.get_atoms()
                            if "H" not in j.get_name()
                        ]
                    )
                )
                ...
                for j, res2 in enumerate(residues):
                    if j == i:
                        continue
                    # print i+1, j+1,
                    new_hbond_dist = np.linalg.norm(
                        res["H"].coord.astype(float) - res2["O"].coord.astype(float)
                    )
                    new_hbond_angle = np.rad2deg(
                        min(
                            [
                                Bio.PDB.calc_angle(
                                    res["N"].get_vector(),
                    ...
                    if (new_hbond_dist < hbond_length) and (
                        new_hbond_angle > hbond_angle
                    ):
                        hbonds.append(i)
                        all_hbonds.append(i)
                        all_hbonds.append(j)
                        ...
                        # print 'true'
                    for O_atom in [
                        x for x in res2 if x.name[0] == "O" and x.name != "O"
                    ]:
                        if (
                            np.linalg.norm(
                                res["H"].coord.astype(float)
                                - O_atom.coord.astype(float)
                            )
                            < hbond_length
                        ):
                            sc_hbonds.append(i)
                    ...
                    if i < j:
                        if "H" in res and "H" in res2:
                            d = np.linalg.norm(
                                res["H"].coord.astype(float)
                                - res2["H"].coord.astype(float)
                            )
                            if d < 13:
                                self.pairlist.append(
                                    (i + len(self.Nterm), j + len(self.Nterm), d)
                                )
            ...
            hbond_depth = np.array(hbond_depth) * np.array(
                [1 if x in hbonds else 0 for x in range(len(residues))]
            )

            self.abego_string = abego_string(structure)
            self.burial = np.array(atoms_in_radius)
            self.hbond_depth = np.array(
                [0 for x in Nterm] + list(hbond_depth) + [0 for x in Cterm]
            )
            ...
            self.hbond_partners = (
                [None for x in Nterm] + hbond_partners + [None for x in Cterm]
            )

            self.sc_hbonds = np.array(
                [0 for x in Nterm]
                + [1 if x in sc_hbonds else 0 for x in range(len(residues))]
                + [0 for x in Cterm]
            )
    ...
    def calculate_approx_rates(self):
        ...
        return np.array(
            [np.median(hbond_rates) for x in hbond_rates]
            + [np.median(nohbond_rates) for x in nohbond_rates]
        )
    ...
    def calculate_isotope_dist(self, undeut, n_isotopes=None, use_empirical=False):
        ...
        emp_isotope_dist = self.calculate_empirical_isotope_dist(
            undeut, n_isotopes=n_isotopes
        )
        ...
        return (
            np.linalg.norm(
                np.dot(theo_isotope_dist[0:minlen], emp_isotope_dist[0:minlen])
            )
            / np.linalg.norm(theo_isotope_dist)
            / np.linalg.norm(emp_isotope_dist)
        )

    def calculate_empirical_isotope_dist(self, undeut, n_isotopes=None):
        isotope_dist = undeut["major_species_integrated_intensities"] / max(
            undeut["major_species_integrated_intensities"]
        )
        isotope_dist = isotope_dist / max(isotope_dist)
        self.isotope_dist = (
            isotope_dist if n_isotopes == None else isotope_dist[0:n_isotopes]
        )
    ...
    def calculate_backexchange(self, fulldeut):
        out = fmin_powell(
            lambda bx: rmse(
                fulldeut["major_species_integrated_intensities"]
                / max(fulldeut["major_species_integrated_intensities"]),
                isotope_poibin_dist(
                    self.isotope_dist,
                    1e9,
                    self.rates,
    ...
    def data_rate_rmse(self, timepoints, rates, concat_data, nbins, backexchange):
        concat_model = isotope_poibin_dist_rates_at_timepoints(
            timepoints, rates, nbins, self.isotope_dist, backexchange
        )
        return mean_squared_error(
            concat_data[concat_data > 0], concat_model[concat_data > 0]
        )

    def fit_data_scalar(self, timepoints, data, init_fes):
        scale = fmin_powell(
            lambda scale: self.data_rmse(
                timepoints, data, np.where(init_fes == -1, -1, init_fes * scale[0])
            ),
            1,
            disp=False,
        )
    ...
    def fit_data(
        ...
        if scalar_init:
            init_fes = self.fit_data_scalar(timepoints, data, init_fes)

        if corr_values == None or corr_weight == 0:
            opt = fmin_powell(
                lambda fes: self.data_rmse(
                    timepoints,
                    data,
                        ...
                        * (
                            np.average(
                                (
                                    (fes[0:-1] - fes[1:])
                                    * np.maximum(
                                        corr_values[0:-1] * corr_values[1:], 0.2
                                    )
                                )
                                ** 2.0
                            )
    ...
    def fit_rates(
        ...
        out = basinhopping(
            lambda rates: self.data_rate_rmse(
                timepoints,
                np.concatenate((rates, fixed_rates)),
                concat_data,
                nbins,
                fitbx,
            ),
    ...
    def fit_rates_and_backexchange(
        self,
        timepoints,
        init_rates=[],
        init_bx_rates=[],
        ...
        if len(init_bx_rates) == 0 and len(init_rates) == 0:
            init_bx_rates = np.concatenate(
                (
                    [logit(self.backexchange)],
                    np.linspace(1, -12, len([x for x in self.rates if x != 0])),
                )
            )
        elif init_rates != 0:
        ...
        out = basinhopping(
            lambda bx_rates: self.data_rate_rmse(
                timepoints,
                np.concatenate((bx_rates[1:], fixed_rates)),
                concat_data,
                nbins,
                expit(bx_rates[0]),
            ),
    ...
    def fit_rates_withfast(
        ...
        out = basinhopping(
            lambda rates: self.data_rate_rmse(
                timepoints,
                np.concatenate((rates, fast_rates)),
                concat_data,
                nbins,
                self.backexchange,
            ),
...
def make_plots(data, p, fit, title, fn):
    ...
    plt.bar(
        range(50),
        data["integrated_intensities"][0] / max(data["integrated_intensities"][0]),
        alpha=0.2,
        color="blue",
    )
    plt.text(
    ...
    for i in range(2, 15):
        ...
        plt.bar(
            range(50),
            data["major_species_integrated_intensities"][i]
            / max(data["major_species_integrated_intensities"][i]),
            alpha=0.3,
            color="blue",
        )

        mytp = float(timepoints[i - 1]) if i < 14 else 1e12
        fithist = isotope_poibin_dist(
            p.isotope_dist,
            mytp,
            p.rates,
            fit,
    ...
    deuts = [
        x - data["major_species_centroid"][0]
        for x in data["major_species_centroid"][1:]
    ]
    ...
    plt.text(
        0.02,
        0.98,
        "Maximum labeling: %.1f%%" % (100 * p.backexchange),
        transform=ax.transAxes,
        horizontalalignment="left",
        verticalalignment="top",
        fontsize=10,
    ...
    plt.scatter(
        np.arange(len(fit))[p.allowed] - len(p.Nterm) + 1,
        fit[p.allowed],
        facecolor=np.where(p.hbonds[p.allowed] == 1, "blue", "none"),
        linewidth=1.0,
    )
    ...
    plt.title(
        "Energy levels:  " + "  ".join(["%.2f" % x for x in extrema]) + " (kcal/mol)"
    )
    ...
    sns.regplot(raveldata[raveldata > 0], ravelmodel[raveldata > 0])
    ...
    plt.text(
        ...
        "R^2 %.2f\nRMSE %.2f"
        % (
            np.corrcoef(raveldata[raveldata > 0], ravelmodel[raveldata > 0])[0][1]
            ** 2.0,
            rmse(raveldata[raveldata > 0], ravelmodel[raveldata > 0]),
        ),
        transform=ax.transAxes,
        horizontalalignment="left",
        verticalalignment="top",
...
def prep_sa_input(p, rect, rates=[], active_rates=[]):
    ...
    too_fast = len([x for x in p.Nterm if x != "P"]) + len(
        [x for x in p.Cterm if x != "P"]
    )
    ...
    if active_rates == []:
        active_rates = np.exp(
            sorted(rates)[too_slow : (len(rates) if too_fast == 0 else -too_fast)]
        )
    else:
    ...
    for aa1, aa2, d in zip(active_aa1s, active_aa2s, new_pairlist[:, 2]):
        for rate_idx1 in range(n):
            for rate_idx2 in range(n):
                pair_energies[aa1, aa2, rate_idx1, rate_idx2] = -np.log(
                    rect(d, abs(fe_grid[aa1, rate_idx1] - fe_grid[aa2, rate_idx2]))
                )
...
class hx_mapping(Annealer):
    ...
    def energy(self):
        ...
        return (
            self.weights["pair_e"] * self.pair_e()
            + (self.weights["full_burial_e"] * self.full_burial_e())
            + (self.weights["hbond_burial_e"] * self.hbond_burial_e())  # 75
            + (self.weights["hbond_rank_e"] * self.hbond_rank_e())  # 50
            + (self.weights["top_stdev"] * self.top_stdev())  # 100
            + (self.weights["distance_to_ss_e"] * self.distance_to_ss_e())
            + (self.weights["distance_to_nonpolar_e"] * self.distance_to_nonpolar_e())
        )  # 80
    ...
    def load_input(self, prepped):
        ...
        self.hbond_ranks_best = sum(
            np.arange(self.n - sum(self.protein.hbonds), self.n)
        )
    ...
    def full_burial_e(self):
        ...
        if min(curr_fes) == max(curr_fes):
            return 0.0
        return -np.corrcoef(
            np.maximum(curr_fes, 0),
            np.maximum(60, self.protein.burial[self.protein.idx_to_full]),
        )[0][1]

    def hbond_burial_e(self):
        curr_fes = np.array([self.fe_grid[aa][i] for aa, i in enumerate(self.state)])
        hbond_fes = curr_fes[self.protein.hbonds[self.protein.idx_to_full] == 1]
        if min(hbond_fes) == max(hbond_fes):
            return 0.0
        return -np.corrcoef(
            np.maximum(hbond_fes, 0),
            np.maximum(
                60,
                self.protein.burial[
                    (self.protein.hbonds == 1) & (self.protein.allowed) == True
                ],
            ),
        )[0][1]
    ...
    def distance_to_nonpolar_e(self):
        ...
        return -np.corrcoef(
            np.maximum(curr_fes, 0),
            self.protein.distance_to_nonpolar[self.protein.idx_to_full],
        )[0][1]
    ...
    def distance_to_ss_e(self):
        ...
        return -np.corrcoef(
            np.maximum(curr_fes, 0),
            self.protein.distance_to_ss[self.protein.idx_to_full],
        )[0][1]
    ...
    def hbond_rank_e(self):
        ...
        return -(float(sum(hbond_ranks)) - self.hbond_ranks_worst) / (
            self.hbond_ranks_best - self.hbond_ranks_worst
        )
