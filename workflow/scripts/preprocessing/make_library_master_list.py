...
def pred_time(x, stretched_times, lo_time, hi_time, lc_timepoints):
    time = int(((x - lo_time)/(hi_time-lo_time))*lc_timepoints) ###previously hardcoded as lo=3, hi=17, diff=987 (987 not correct, but not really harmful, ~1-2% error)
...
def cluster(df, name_dict, key, RT_cutoff):
    ...
    clusters = [[j for j in n_df['idx'].values if abs(n_df.loc[n_df['idx']==i]['pred_RT'].values[0]-n_df.loc[n_df['idx']==j]['pred_RT'].values[0]) < RT_cutoff] for i in n_df['idx'].values]
    no_dups = []
...
def intersection_filter(final, intersections, n_df):
    ...
    [[final_copy[i].discard(j) for j in intersections] for i in range(len(final_copy))] #modifies final_copy in place, removing intersection values from each cluster in final_copy
    means = [np.mean(n_df.loc[n_df['idx'].isin(list(st))]['pred_RT'].values) for st in final_copy] #generates mean pred_RT of each mutualy exclusive cluster, order maps to final_copy
    dists = [[abs(n_df.loc[n_df['idx']==i]['pred_RT'].values-mean) for mean in means] for i in intersections] #outer order maps to intersections, inner maps to final_copy
    [final_copy[dists[i].index(min(dists[i]))].add(intersections[i]) for i in range(len(intersections))] #adds value from intersection to best-fitting cluster
    return final_copy
...
tics = [np.loadtxt(fn) for fn in ins if ".ims.mz.tic" in fn]

for i in range(len(tics)):
    tics[i] = tics[i] / (np.sum(tics[i],axis=0) + 1)

#used in pred_time
lo_time, hi_time, lc_timepoints = set_global_scan_bounds(mzml)

#Compute minimum-cost warp-path for each timepoint paired with the first undueterated
#Assumes Undeuterated comes first in input, may need to revise
paths=[]
for i in range(0,len(tics)):
    distance, path = fastdtw(tics[0].T, tics[i].T, dist = euclidean, radius = 20) #Hardcoded fdtw radius for best time-efficiency/accuracy
    paths.append(path)

#applies transform to test pattern (non UN0 timeseries) for ts1, applies transform to UN0 for ts2 entries
#That is, ts2 estimates where proteins seen in UN0 should show up in later runs, and ts1 is used to map undeuterated replicate peaks back to UN0 RT (pred_RT)
stretched_ts1_times=[]
stretched_ts2_times=[]
for path in paths:
    stretched_ts1_times.append(path_to_stretch_times(path, 0))
    stretched_ts2_times.append(path_to_stretch_times(path, 1))


plt.figure(figsize=(20,13))
print (len(tics), len(stretched_ts1_times))
for tic, stretched in zip(tics, stretched_ts1_times):
    plt.plot(stretched-np.arange(len(stretched)), label=tic)
plt.legend()
...
for i in range(len(undfs)):
    undfs[i]['pred_RT'] = [pred_time(x, stretched_ts1_times[i], lo_time, hi_time, lc_timepoints) for x in undfs[i]['RT']]
...
for i in range(1,len(catdf)):
    if ((catdf['name'].values[i] == catdf['name'].values[i-1]) and
        ...
        (abs(catdf['pred_RT'].values[i] - catdf['pred_RT'].values[i-1]) < snakemake.config['rt_group_cutoff'])): #ensures no duplicate charge-states make it into same rt-group
        dups.append(True)
    else:
        dups.append(False)
catdf['dup'] = dups
catdf=catdf.query('dup == False')
catdf['sequence'] = [list(name_and_seq.loc[name_and_seq['name'] == catdf.iloc[i]['name']]['sequence'].values)[0] for i in range(len(catdf))] #adds sequences to output
catdf['idx'] = [i for i in range(len(catdf))]

#cluster RT values and rename 
name_dict = OrderedDict.fromkeys(catdf['name'].values)
[cluster(catdf, name_dict, key, snakemake.config['rt_group_cutoff']) for key in name_dict.keys()] #TODO possibly automate rt_group cutoff determination in the future
...
for i in range(len(catdf)):
    if catdf.iloc[i]['name'] != prev_name:
        ...
        [[all_tp_mean_preds[i].append(name_rt_preds[i]) for i in range(len(all_tp_mean_preds))] for j in range(len(subdf))]
