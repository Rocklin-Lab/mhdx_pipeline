...
tp_inputs = [[
    tensor for tensor in snakemake.input if snakemake.config[tp][0].split("/")
    [-1].split(".")[0].split("_")[-1] in tensor
] for tp in snakemake.config["timepoints"]]

t1 = hx.TensorGenerator(name, library_info, tp_inputs,
                        snakemake.config["timepoints"])
t1.generate_tensors()
